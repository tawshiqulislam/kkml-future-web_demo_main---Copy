...
class CheckoutController extends Controller
{
        ...
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('order_id') != null) {
                if ($request->payment_option == 'paypal') {
                    ...
                    return $bkash->pay();
                } elseif ($request->payment_option == 'smanager') {
                    $smanagerPay = new sManagerPaymentController;
                    return $smanagerPay->pay();
    ...
    public function checkout_done($order_id, $payment)
    {
        ...
        $single_order = OrderDetail::where('order_id', $order_id)->first();
        $order->payment_status = 'paid';
        $order->payment_details = $payment;
        $user = User::where('id',$order->user_id)->first();
        if($user->referred_by != null && $user->membership == "approved"){
            $referred_by_user = User::where('id',$user->referred_by)->first();
            $order->commission_to = $referred_by_user->id;
            $commission = $single_order->price * (2 / 100);
            $order->commission_calculated = $commission;
            $referred_by_user->balance = $referred_by_user->balance  + $commission;
        ...
        if (Addon::where('unique_identifier', 'seller_subscription')->first() == null ||
            !Addon::where('unique_identifier', 'seller_subscription')->first()->activated) {
...
//            }
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        foreach (Session::get('cart') as $key => $cartItem) {
            ...
            $shipping_arr->push(calculate_shipping($cartItem['shipping_cost'], $cartItem['product_shipping_type'], $shipping_info['city'], $cartItem['is_quantity_multiplied'], $cartItem['quantity']));
        }

        $shipping = $shipping_arr->max();
    ...
    {
        if (Addon::where('unique_identifier', 'club_point')->first() != null &&
            Addon::where('unique_identifier', 'club_point')->first()->activated) {
