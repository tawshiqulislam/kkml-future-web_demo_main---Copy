...
class HomeController extends Controller
{
    ...
    {
        $user = User::whereIn('user_type', ['customer', 'seller'])->where('email', $request->email)->orWhere('phone', "+880" . substr($request->email, -10))->first();
        if ($user != null) {
    ...
    public function customer_update_profile(Request $request)
    {
        ...
        $user->phone = "+880" . substr($request->phone, -10);
    ...
    public function paginate_all_product(Request $request)
    {
        if (!$request->has('page') || $request->page <= 1) {
            \Session::put('chunks', rand());
        }

        $products = filter_products(Product::where('published', 1)->inRandomOrder())->paginate(12);
        return view('frontend.partials.ajax_all_products', compact('products'));
    }

    public function load_all_product_section()
    {
        return view('frontend.partials.all_product');
    }

    public function slide_by_category($categoryId)
    {
        dd('Method called with ID: ' . $categoryId);
        return 1;
        ...
        return view('frontend.partials.slide_product_by_category', compact('category', 'products'));
    }

    public function load_all_service()
    {
        $services = Service::where('published', 1)->inRandomOrder()->paginate(16);
        return view('frontend.partials.all_service', compact('services'));
    }
    ...
    {
        $detailedService = Service::where('id', $id)->first();

        $last = Product::where('user_id', $detailedService->user_id)->where('published',1)->inRandomOrder()->first();

        $detailedProduct = Product::where('slug', $last->slug)->first();
        if($detailedService != null){
            return view('frontend.service_details', compact('detailedProduct', 'detailedService'));
        }
        abort(404);
    }

    public function service_appoint(Request $request)
    {
        if($request->has('service_id')){
            $service_order = new ServiceOrder;
            $service_order->service_id = $request->service_id;
            $service_order->user_info = $request->id;
            $service_order->name = $request->name;
            $service_order->email = $request->email;
            $service_order->phone = $request->phone;
            $service_order->status = 'New';
            $service_order->code = date('Ymd-His') . rand(10, 99);
            if($service_order->save()){
                Session::flash('success','Appoint Request Send');
                return back();
            }
        }
    }
...
//        $categories = Category::where('level', 0)->orderBy('name', 'asc')->get();
    ...
    {
        $categories = Category::all();
        return view('frontend.all_author', compact('categories'));
    }
