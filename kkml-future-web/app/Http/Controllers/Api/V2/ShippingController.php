...
//            }
            ...
            if ($product) {
                $product_shipping_cost = calculate_shipping($product->shipping_cost, $product->shipping_type, $request->city_name, $product->is_quantity_multiplied, $shop_items_raw_data_item->quantity);
                $shipping_arr->push($product_shipping_cost);
        ...
        return response()->json(['result' => true, 'shipping_type' => get_setting('shipping_type'), 'value' => convert_price($shipping_arr->max()), 'value_string' => format_price($shipping_arr->max())], 200);
    }
