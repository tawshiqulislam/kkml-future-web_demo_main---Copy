...
if ( ! function_exists( 'sendSMS' ) ) {
	...
	function sendSMS( $to, $from, $text ) {
		...
		} elseif ( OtpConfiguration::where( 'type', 'green_web' )->first()->value == 1 ) {

			$to = substr( $to, -13 );
			// send sms by alpha net
			try {
				$params = [
...
//
//            $token = env('GREEN_WEB_API_TOKEN') ?? config('app.GREEN_WEB_API_TOKEN');
//
//            $data = array(
//                'to' => substr($to, -11),
//                'message' => "$text",
...
if ( ! function_exists( 'filter_products_by_category' ) ) {
	function filter_products_by_category( $products ) {
		$verified_sellers = verified_sellers_id();

		$query = $products->where( 'published', '1' )->orderBy( 'created_at', 'desc' )->where( function ($p) use ($verified_sellers) {
			$p->where( 'added_by', 'admin' )->orWhere( function ($q) use ($verified_sellers) {
				$q->whereIn( 'user_id', $verified_sellers );
			} );
		} );
...
if ( ! function_exists( 'home_discounted_base_price' ) ) {
	function home_discounted_base_price( $id ) {
		...
		$flash_deals = FlashDeal::where( 'status', 1 )->where( 'start_date', '<=', time() )->where( 'end_date', '>=', time() )->get();
		...
		foreach ( $flash_deals as $flash_deal ) {
			...
			if ( $flash_deal != null && $flash_deal_product != null ) {
				if ( $flash_deal_product->discount_type == 'percent' ) {
...
if ( ! function_exists( 'flash_deal_product' ) ) {
	function flash_deal_product( $id ) {
		$flash_deals = FlashDeal::where( 'status', 1 )->where( 'start_date', '<=', time() )->where( 'end_date', '>=', time() )->get();

		$flash_deal_product = null;
		foreach ( $flash_deals as $flash_deal ) {
			$flash_deal_product = FlashDealProduct::where( 'flash_deal_id', $flash_deal->id )->where( 'product_id', $id )->first();

			if ( $flash_deal != null && $flash_deal_product != null ) {
				return $flash_deal_product;
			}
		}
...
if ( ! function_exists( 'session_rand' ) ) {
	function session_rand() {
		if ( session()->has( 'rand' ) ) {
...
if ( ! function_exists( 'calculate_shipping' ) ) {
	function calculate_shipping( $shipping_cost, $shipping_type = 'free', $city = null, $multiplied = 0, $quantity = 1 ) {
		$shipping = 0;


		if ( $shipping_type == 'flat_rate' ) {
			$shipping = (double) $shipping_cost;
		}

		if ( $shipping_type == 'product_wise' ) {
			if ( is_array( json_decode( $shipping_cost, true ) ) ) {
				foreach ( json_decode( $shipping_cost, true ) as $shipping_region => $val ) {
					if ( $city == $shipping_region ) {
						$shipping = (double) ( $val );
					}
				}
			}
