...
@section('panel_content')
    ...
    @if(Auth::user()->referral_code != Null)
        <div class="row gutters-4 mb-2">
            <div class="col">
                <div class="h6 fw-500">
                    <div class="r-tag" style="font-size: 15px">
                        <b>{{ translate('Referral code') }}</b> <input class="ml-2" type="text" value="{{Auth::user()->referral_code}}" id="myInput"
                        style="
                        width: 16%;
                        text-align: center;
                        font-size: 18px;
                        border-radius: 50px"
                        readonly>
                        <button type="button" class="btn btn-light" onclick="myFunction()" onmouseout="outFunc()"
                        title="Copy to clipboard"
                        style="
                        border-radius: 50px;
                        padding: 0;
                        margin-bottom: 5px;
                        background-color: rgba(9, 255, 0, 0.397)">
                            <span id="myTooltip">
                                <i class="las la-copy m-1" style="width: 22px"></i>
                            </span>
                        </button>
                    </div>
                    <div style="font-size: 15px">
                        <small><b>{{ translate('Referred By: ') }}<i class="mx-4">{{App\User::find(Auth::user()->referred_by)->name}}</i></b></small>
                    </div>
                </div>
			...
			<div class="card">
		        ...
		        <div class="card-body">
                  @if(Auth::user()->addresses != null)
                      @php
                          $address = Auth::user()->addresses()->where('set_default', 1)->orderBy('id', 'desc')->first();
                      @endphp
                      @if($address != null)
...
{{--                              <li class=" py-2"><span>{{ translate('Postal Code') }} : {{ $address->postal_code }}</span></li>--}}
    ...
    <script>
        function myFunction() {
          ...
          tooltip.innerHTML = '<i class="las la-check m-1" style="width: 50px"></i>';
        }

        function outFunc() {
          var tooltip = document.getElementById("myTooltip");
          tooltip.innerHTML = '<i class="las la-copy m-1" style="width: 22px"></i>';
        }
        </script>
@endsection
