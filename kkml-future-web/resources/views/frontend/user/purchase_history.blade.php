                ...
                <table class="table aiz-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th data-breakpoints="md">{{ translate('Product & Review') }}</th>
                    ...
                    <tbody>
                        @foreach ($orders as $key => $order )
                            @if (count($order->orderDetails) > 0)
                                {{-- @php
                                    $orderInfo = App\OrderDetail::where('order_id', $order->id)->first();
                                    $productInfo = App\Product::select('name', 'photos')
                                        ->where('id', $orderInfo->product_id)
                                        ->first();
                                    $arr = explode(',', $productInfo->photos);
                                    $firstImg = $arr[0];
                                @endphp --}}
                                <tr>
                                    <td>
                                    ...
                                    <td style="overflow:hidden;">
                                        <a href="{{route('product', App\Product::where('id', App\orderDetail::where('order_id', $order->id)->first(['product_id'])->product_id)->first(['slug'])->slug)}}">{{App\Product::where('id', App\orderDetail::where('order_id', $order->id)->first(['product_id'])->product_id)->first(['name'])->name}}</a>
                                        <div>
                                            <small><b>{{App\orderDetail::where('order_id', $order->id)->first(['price'])->price}}</b> &#2547;</small>
                                        </div>
                                    </td>
                                    {{-- <td>
                                        @if (1 == 1)
                                            <a href=""><span class="badge badge-inline badge-warning">
                                                {{ translate('Post a Review') }}
                                                {{App\Product::where('id', $order->orderDetails->product_id)->first()->slug}}
                                            </span></a>
                                        @else
                                            <span class="badge badge-inline badge-info">
                                                {{ translate('Review Posted') }}
                                            </span>
                                        @endif
                                    </td> --}}
                                    <td class="text-right">
