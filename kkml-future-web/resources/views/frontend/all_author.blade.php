    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-6 text-center text-lg-left">
                <h1 class="fw-600 h4">{{ translate('All Authors') }}</h1>
            </div>
            <div class="col-lg-6">
                <ul class="breadcrumb bg-transparent p-0 justify-content-center justify-content-lg-end">
                    ...
                    <li class="text-dark fw-600 breadcrumb-item">
                        <a class="text-reset" href="{{ route('authors.all') }}">"{{ translate('All Authors') }}"</a>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div class="container">
        <div class="bg-white shadow-sm rounded px-3 pt-3">
            <div class="row row-cols-xxl-6 row-cols-xl-5 row-cols-lg-4 row-cols-md-3 row-cols-2 gutters-10">
                @foreach (\App\Brand::where('is_author', 1)->get() as $brand)
                    <div class="col text-center">
                        <a href="{{ route('products.brand', $brand->slug) }}" class="d-block p-3 mb-3 border border-light rounded hov-shadow-md">
                            ...
                            <h6 class="text-reset pt-2">{{ $brand->name }}</h6>
                        </a>
                    </div>
                @endforeach
            </div>
