...
@if(Auth::user()->membership == 'approved')

@section('panel_content')
    <div class="aiz-titlebar mt-2 mb-4">
        <div class="row align-items-center">
            <div class="col-md-6">
                <h1 class="h3">{{ translate('Membership Dashboard') }}</h1>
            </div>
        </div>
    </div>
    <div class="aiz-titlebar mt-2 mb-4">
    ...
    <div class="row gutters-10">
		<div class="col-md-4">
			<div class="bg-grad-1 text-white rounded-lg mb-4 overflow-hidden">
				<div class="px-3 pt-3">
                        <div class="h3 fw-700"> {{ translate('Referrel Code') }}</div>
                        <div style="color:black; font-weight:bold">{{Auth::user()->referral_code}}</div>
					</div>
		...
		<div class="col-md-4">
			<div class="bg-grad-2 text-white rounded-lg mb-4 overflow-hidden">
				<div class="px-3 pt-3">
                    <div class="h3 fw-700"> {{ translate('Balance') }}</div>
                    <div style="color:black; font-weight:bold">{{Auth::user()->balance}} BDT</div>
                </div>
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
		...
		<div class="col-md-4">
			<div class="bg-grad-3 text-white rounded-lg mb-4 overflow-hidden">
				<div class="px-3 pt-3">
                    <div class="h3 fw-700"> {{ translate('User Referred') }}</div>
					<div style="color:black; font-weight:bold"><span class="opacity-50">{{translate('Total:')}}</span> {{Auth::user()->where('referred_by', Auth::user()->id)->count()}}</div>
				</div>
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
...
@section('panel_content')
    ...
    <div class="row gutters-10">
		<div class="col-md-4 bg-grad-1 rounded-lg mb-3">
			<div class="text-white overflow-hidden">
				Membership Rejected. Contact for more details.
			</div>
		</div>
	</div>
@endsection
...
@section('panel_content')
    ...
    <div class="row gutters-10">
		<div class="bg-grad-1 text-white rounded-lg mb-4 overflow-hidden">
			<div class="px-3 p-3">
                <div class="text-white overflow-hidden">
					Please wait until someone accept your membership. Feel free to cantact for any issue
				</div>
			</div>
			<div class="px-3 p-3">
                <div class="text-white overflow-hidden">
					<a href="{{ route('membership.cancel', Auth::user()->id) }}" class="btn btn-circle btn-info top-75 left-75">
						<span>{{translate('Withdraw Membership Request')}}</span>
					</a>
				</div>
			</div>
		</div>
